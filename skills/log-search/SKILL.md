...
2. Prefer the local script (relative to this skill directory):
   `python3 scripts/fetch_index_docs.py`
...
7. For `fetch_index_docs.py`, `-h` means human-readable aligned table output. Use `--help` when you need command help.
8. Use `--output message` when the user only wants the raw log message field with timestamp and pod.
9. Use `--output compact` when JSON structure is helpful and `--output full` only when needed.
10. Use structured filters for Kubernetes metadata:
...
11. CSV output is intentionally minimal:
...
12. Compact JSON output is intentionally minimal:
...
- For data view mappings, read:
  [dataviews.py](scripts/dataviews.py)
